...
func _enter_dock():
	...
	for entry in level_list.levels_ordered:
		add_item(entry.level_data, entry.is_active)
	update_list()


func update_list():#read folder and update list
...
func add_item(level_data:LevelData, is_checked:bool = true):
	...
	item.set_tooltip(1, level_data.scene.resource_path)
	item.add_button(1, edit_icon, -1, false,
		str("Edit Level (", level_data.scene.resource_path, ")"))
	item.add_button(1, play_icon, -1, false,
		str("Play Level (", level_data.scene.resource_path, ")"))
...
func _on_item_moved(from_index:int, to_index:int):
	level_list.move_level(from_index, to_index)

func _on_button_pressed(item:TreeItem, column:int, id:int):
	match id:
		0:
			emit_signal("open_scene", (item.get_meta("level_data") as LevelData).scene)
		1:
			emit_signal("run_scene", (item.get_meta("level_data") as LevelData).scene)
